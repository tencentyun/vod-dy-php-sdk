 ...
 * 图片水印模板输入参数
 *
 * @method MediaInputInfoForDY getImageContent() 获取水印图片的输入内容。支持 jpeg、png 图片格式。
 * @method void setImageContent(MediaInputInfoForDY $ImageContent) 设置水印图片的输入内容。支持 jpeg、png 图片格式。
...
<li>当字符串以 px 结尾，表示水印 Height 单位为像素，如 100px 表示 Height 为 100 像素。</li>
默认值：0px，表示 Height 按照原始水印图片的宽高比缩放。
 * @method string getRepeatType() 获取印重复类型。使用场景：水印为动态图像。取值范围：
<li>once：动态水印播放完后，不再出现；</li>
<li>repeat_last_frame：水印播放完后，停留在最后一帧；</li>
<li>repeat：水印循环播放，直到视频结束（默认值）。</li>
 * @method void setRepeatType(string $RepeatType) 设置印重复类型。使用场景：水印为动态图像。取值范围：
<li>once：动态水印播放完后，不再出现；</li>
<li>repeat_last_frame：水印播放完后，停留在最后一帧；</li>
<li>repeat：水印循环播放，直到视频结束（默认值）。</li>
 */
class RawImageWatermarkInputForDY extends AbstractModel
{
    /**
     * @var MediaInputInfoForDY 水印图片的输入内容。支持 jpeg、png 图片格式。
     */
    public $ImageContent;
...
默认值：0px，表示 Height 按照原始水印图片的宽高比缩放。
    ...
    /**
     * @var string 印重复类型。使用场景：水印为动态图像。取值范围：
<li>once：动态水印播放完后，不再出现；</li>
<li>repeat_last_frame：水印播放完后，停留在最后一帧；</li>
<li>repeat：水印循环播放，直到视频结束（默认值）。</li>
     */
    ...
    /**
     * @param MediaInputInfoForDY $ImageContent 水印图片的输入内容。支持 jpeg、png 图片格式。
...
<li>当字符串以 px 结尾，表示水印 Height 单位为像素，如 100px 表示 Height 为 100 像素。</li>
默认值：0px，表示 Height 按照原始水印图片的宽高比缩放。
     * @param string $RepeatType 印重复类型。使用场景：水印为动态图像。取值范围：
<li>once：动态水印播放完后，不再出现；</li>
<li>repeat_last_frame：水印播放完后，停留在最后一帧；</li>
<li>repeat：水印循环播放，直到视频结束（默认值）。</li>
     */
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("ImageContent",$param) and $param["ImageContent"] !== null) {
            $this->ImageContent = new MediaInputInfoForDY();
